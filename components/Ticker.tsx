export const fallbackTickerMessages = [
...
function clampTickerSpeed(speedSeconds: number) {
  if (!Number.isFinite(speedSeconds)) return 30;
  return Math.min(90, Math.max(10, Math.round(speedSeconds)));
}

function TickerSegmentGroup({
  isHidden = false,
  messages,
}: {
  isHidden?: boolean;
...
export function Ticker({
  messages = fallbackTickerMessages,
  speedSeconds = 30,
}: {
  messages?: string[];
  speedSeconds?: number;
}) {
  const tickerMessages = messages.length > 0 ? messages : fallbackTickerMessages;
  const tickerSpeed = clampTickerSpeed(speedSeconds);
  ...
  return (
    <div className="overflow-hidden border-y border-[#d7a84f]/30 bg-[#080604]/68 text-[#f4d28b] shadow-[0_18px_40px_rgba(0,0,0,0.25)] backdrop-blur-md">
      <div
        className="flex w-max animate-[ticker_linear_infinite] whitespace-nowrap py-3 text-xs font-semibold uppercase tracking-[0.18em] sm:text-sm sm:tracking-[0.2em]"
        style={{ animationDuration: `${tickerSpeed}s` }}
      >
        <TickerSegmentGroup messages={tickerMessages} />
        <TickerSegmentGroup messages={tickerMessages} isHidden />
        <TickerSegmentGroup messages={tickerMessages} isHidden />
      </div>
    </div>
  );
